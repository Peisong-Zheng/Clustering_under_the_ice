...
def plot_san_temp_profiles(ds)-> None:
    """
    ...
    for i in sequence:
        ...
        # # find the profile with salinity>40
        # pro_index=np.where(ds_class['salinity']>40)
        # if pro_index[0].size>0:
        #     pro_index=np.unique(pro_index[0])
        #     pro_index=ds_class['nprof'][pro_index].values
        #     print(f'nprof= {pro_index} in class {i} with salinity>40, excluded')
        #     # drop the profile with salinity>40
        #     ds_class=ds_class.drop_sel(nprof=pro_index)

        
        
        # pro_index=np.where(ds_class['salinity']<5)
        # if pro_index[0].size>0:
        #     pro_index=np.unique(pro_index[0])
        #     pro_index=ds_class['nprof'][pro_index].values
        #     print(f'nprof= {pro_index} in class {i} with salinity<5, excluded')
        #     # drop the profile with salinity>40
        #     ds_class=ds_class.drop_sel(nprof=pro_index)
